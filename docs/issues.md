...
- **`rules edit <index>`** — change action (`--action`), access direction
  (`--access`), or re-pin the binary hash (`--repin` / `--no-pin`).
...
- **`rules import`** — merge rules from TOML on stdin, skipping exact
  duplicates.
- **Separated ownership** — declarative config rules are read-only; learned
  rules live in a SQLite repository and are mutated through a versioned daemon
  control API. Stable database IDs back edit/remove operations, and exact
  duplicates are rejected across both rule sources.
...
## 10. ~~Index-based config mutation can target stale rule positions~~ ✅ RESOLVED

Runtime rule mutations no longer rewrite indexed `[[rule]]` tables. The CLI
resolves a displayed learned rule to its stable database ID, then sends that ID
and a field patch to the daemon for the serialized mutation. It never writes a
stale whole-rule snapshot. Declarative rules are read-only.

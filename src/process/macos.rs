...
/// Process argv. TODO(macOS): read via `sysctl(KERN_PROCARGS2)`. Until then
/// script-identity is unavailable on macOS.
pub fn cmdline(_pid: u32) -> anyhow::Result<Vec<String>> {
    Ok(Vec::new())
}
